...
if (! function_exists('format_price')) {
    function format_price(
        ...
        if ($useSymbol && $currency->is_prefix_symbol) {
            $space = setting('car_rentals_add_space_between_price_and_currency', 0) == 1 ? ' ' : null;
...
if (! function_exists('human_price_text')) {
    function human_price_text(float|null|string $price, Currency|null|string $currency, ?string $priceUnit = '', bool $fullNumber = false): string
    {
        ...
        if (! $fullNumber && setting('car_rentals_convert_money_to_text_enabled', false)) {
        ...
        $decimalSeparator = setting('car_rentals_decimal_separator', '.');

        if ($decimalSeparator == 'space') {
            $decimalSeparator = ' ';
        }

        $thousandSeparator = setting('car_rentals_thousands_separator', ',');
